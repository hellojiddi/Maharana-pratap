    ...
    >
      <div className="max-w-[1400px] mx-auto px-6 h-full flex items-center justify-between">
        ...
        <div className="hidden lg:flex items-center gap-8">
          {navLinks.map((link) => (
            <Link
              key={link.path}
              to={link.path}
              className={`text-[14px] uppercase tracking-[0.06em] font-body transition-colors duration-300 ${
                isActive(link.path)
                  ? 'text-burnt'
            ...
            >
              {link.label}
              {isActive(link.path) && (
                <span className="block h-[2px] bg-burnt mt-1.5" />
              )}
            </Link>
          ))}
        </div>
        ...
        <div className="hidden lg:block">
          <Link
            to="/contact"
            className="inline-flex items-center px-6 py-2.5 border border-cream text-cream rounded-md text-[13px] uppercase tracking-[0.06em] font-body transition-all duration-300 hover:bg-burnt hover:border-burnt"
          >
            Apply Now
          </Link>
        </div>
